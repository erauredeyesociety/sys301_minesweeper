...
| **G-5** | **TR-5 has nothing to inspect.** | The port map is empty and no code reads it. The requirement is neither satisfied nor violated. | The build exists → operator describes it → port map filled → `sensors.py` reads it → grep-based inspection becomes meaningful. |

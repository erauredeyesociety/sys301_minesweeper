...
| `src/mission_config.py` (recommend) | Add `COUNTDOWN_S = 10`, `START_SLOT = 0`, `TARGET_COLOUR_NAME = "yellow"` (`[ASSUMED]`, label), `HEARTBEAT_MS`, `LOG_EVENTS_ONLY = True`, `TELEMETRY_LIVE_ENABLED = False`. **Set `CLASSES = (TARGET_COLOUR_NAME,)`** so the class name is the label. `RUN_TIMEBOX_S` already exists. **Do not** add `HOLD_STOP_MS` or `TARGET_HINT_TOLERANCE` (dropped as over-engineering, §3.5). |

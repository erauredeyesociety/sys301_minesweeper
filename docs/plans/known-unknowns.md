...
`DETECT_MODE` in [config.py](../../src/config.py) still defaults to `"anomaly"` and `main.py` still
...
2. **KU-D11 — make `src/hub_color.py` read `SECOND_COLOR_PORT`.** A known defect with a known fix, and
   at 10 ft it is the difference between 75 lanes and 38. Then KU-M29.

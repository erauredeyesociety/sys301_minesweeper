...
- [x] ~~**`floor_anomaly` detection validated** — 0%-false on real floor data~~ — ⚠ **SUPERSEDED 2026-09-08.** That result stands for *that* floor, but on the **real classroom carpet** the chromaticity front-end **fails**: yellow INVISIBLE (0 % above threshold), blue tape a 100 % false positive, cause = quantisation (carpet totals ~79 ADC counts). **Replaced by `reflection() >= 30`** — zero overlap, 43-point gap, colour-agnostic — [findings/colour-survey-and-first-detection-2026-09-08.md](./findings/colour-survey-and-first-detection-2026-09-08.md)
- [x] ✅ **GATE 1 CLOSED 2026-09-08** — a real sticky note found **while moving, untethered on battery, twice**, both colours correctly classified (`PINK refl=99`, `YELLOW refl=62`). Closes KU-M22/M32 — [findings/colour-survey-and-first-detection-2026-09-08.md](./findings/colour-survey-and-first-detection-2026-09-08.md)
- [x] **`deploy_deps.py`** auto-resolves + deploys a program's `src/` imports to `/flash/lib`
- [ ] **Next (hardware), updated 2026-09-08:** ① measure `SENSOR_SPACING_MM` + the fore-aft offset (a ruler, KU-M33) · ② make `src/hub_color.py` read `SECOND_COLOR_PORT` — the mission code is a **one-sensor robot** today (KU-D11/R-19) · ③ first `main.py` run (KU-M29/M30, R-18). Real-mine detection is **DONE**; the units of "10×10" are **provisionally 10 ft** (operator, "not set in stone") — [plans/next-session.md](./plans/next-session.md)
